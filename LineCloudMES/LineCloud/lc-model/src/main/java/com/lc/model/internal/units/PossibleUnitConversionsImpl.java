...
import com.lc.model.api.DictionaryService;
import com.lc.model.api.Entity;
import com.lc.model.api.NumberService;
import com.lc.model.api.units.UnitConversion;
import com.lc.model.api.units.UnsupportedUnitConversionException;
...
public final class PossibleUnitConversionsImpl implements InternalPossibleUnitConversions {
    ...
    private final Map<String ,Boolean> targetUnitToFactor;
    ...
    public void addConversion(final UnitConversion unitConversion) {
        ...
        if (!isDefinedFor(unit)) {
            targetUnitToFactor.put(unitConversion.getUnitFrom(),unitConversion.getRatio());
        }
    }
    ...
    @Override
    public boolean isDefinrdFor(final String unit) {
    ...
    @Override
    public  BigDecimal converTo(final BigDecimal quantityFrom,final String unitTo) {
        final BigDecimal ration = targetUnitToFactor.get(unitTo);
        if (ration == null) {
            throw new UnsupportedUnitConversionException(unitFrom,unitTo);
        }
        BigDecimal convertedValue =quantityFrom.multiply(ration);
        boolean unitIsInteger = targetUnitToInteger.computeIfAbsent(unitTo,dictionaryService::checkIfUnitIsInteger);
        if (unitIsInteger) {
    ...
    public BigDecimal convertTo (BigDecimal quantityFrom,String unitTo,int roundMode) {
        ...
        if (ratio == null) {
            throw new UnsupportedOperationException(unitFrom,unitTo);
        ...
        if (unitIsInteger) {
            ...
            return convertedValue.setScale(NumberService.DEFAULT_MAX_INTEGER_DIGITS_IN_DECIMAL,roundMode);
        }
    }
    ...
    private Entity buildUnitConversionItem(final String unitTo,final BigDecimal,final Entity ownerEntity) {
