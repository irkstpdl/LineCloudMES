...
import com.lc.model.api.PriorityService;
...
public class EntityTreeUtilsService {
    ...
    public static EntityTree getDatachedEntityTree(final List<Entity> entities) {
    ...
    public List<Entity> getSortedEntities(final EntityTree tree) {
        ...
        return getSortedEntitiesFormNode(tree.getRoot());
    }

    private List<Entity> getSortedEntitiesFormNode(final EntityTreeNode node) {
        List<Entity> nodesList = Lists.newLinkedList();
        nodesList.add(node);
        ...
        for (EntityTree childNode : childNodes) {
            nodesList.addAll(getSortedEntitiesFormNode(childNode));
        }
